"""
Server-side export: PDF técnico, PDF gerencial e CSV.
Porta a lógica de desktop_app/export_utils.py para ser chamada pelos endpoints da API.
"""
import csv
import io
...
_DEADLINE_DAYS = {"critical": 7, "high": 30, "medium": 90, "low": 180}

INFRA_PROFILE_TITLE = "Infrastructure & Technology Profile"

CSV_FIELDS = [
    "num_ameaca", "scan_datetime", "severity", "cve", "title", "description",
    "recommendation", "category", "module", "affected_url", "cvss_score",
    "owasp_category", "data_conclusao", "status", "observacao",
]
...
def _action_deadline(severity: str, scan_time: datetime) -> str:
    days = _DEADLINE_DAYS.get(severity)
    return (scan_time + timedelta(days=days)).strftime("%d/%m/%Y") if days else "—"
...
def _executive_paragraph(findings: list[dict], url: str) -> str:
...
def _gerencial_footer(canvas, doc):
    ...
    page_width, _ = A4
    text = (
        f"ThreatLens v{settings.APP_VERSION} — Gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')} — "
        f"Desenvolvido por {settings.APP_AUTHOR} — CONFIDENCIAL"
    )
    p = Paragraph(text, footer_style)
    p.wrap(page_width - 4 * cm, 1 * cm)
...
def generate_csv(result: dict, scan_time: datetime | None = None) -> bytes:
...
def generate_pdf(result: dict, url: str, scan_time: datetime | None = None) -> bytes:
    ...
    findings = _sort_findings([f for f in all_findings if f.get("title") != INFRA_PROFILE_TITLE])
    ...
    story = [
        ...
        Paragraph(f"Alvo: {_esc(url)}", styles["Normal"]),
        Paragraph(f"Data/Hora do Scan: {scan_time_str}", styles["Normal"]),
        Paragraph(
            f"Risk score: {result.get('risk_score')} | "
            f"Duração: {result.get('duration_ms')} ms | Achados: {len(findings)}",
            styles["Normal"],
        ),
    ]
    ...
    for idx, finding in enumerate(findings, 1):
        ...
        severity_style = ParagraphStyle(
            "sev_cell", parent=cell_style, fontSize=8, leading=10,
    ...
    table = Table(
        table_data,
        colWidths=[0.7 * cm, 2.2 * cm, 2.1 * cm, 3.4 * cm, 5.3 * cm, 4.6 * cm],
        repeatRows=1,
    )
    ...
    story.append(Paragraph(
        f"ThreatLens v{settings.APP_VERSION} — desenvolvido por {settings.APP_AUTHOR}",
        styles["Normal"],
    ))
    doc.build(story)
    return buf.getvalue()
...
def generate_pdf_gerencial(result: dict, url: str, scan_time: datetime | None = None) -> bytes:
    ...
    actionable = _sort_findings([f for f in findings_no_infra if f.get("severity") != "informational"])
    ...
    heading1 = ParagraphStyle("h1g", parent=styles["Heading1"], fontSize=18,
                               textColor=colors.HexColor("#2DD4BF"), spaceAfter=4)
    heading2 = ParagraphStyle("h2g", parent=styles["Heading2"], fontSize=13,
    ...
    story.append(Paragraph(
        f"<b>Risk Score:</b> <font color='{risk_color}'><b>{risk_score} — {_risk_label(risk_score)}</b></font>",
        body,
    ))
    story.append(Spacer(1, 0.5 * cm))
    ...
    story.append(Paragraph(_executive_paragraph(findings_no_infra, url), body))
    story.append(Spacer(1, 0.4 * cm))

    counts = _severity_counts(findings_no_infra)
    ...
    if actionable:
        ...
        for idx, f in enumerate(actionable, 1):
            ...
            action_data.append([
                ...
                Paragraph(_PRIORITY_PT.get(sev, "—"), cell_style),
                Paragraph(_action_deadline(sev, scan_time), cell_style),
            ])
        action_table = Table(
            action_data,
